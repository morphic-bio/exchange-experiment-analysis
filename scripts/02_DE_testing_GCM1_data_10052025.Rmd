...
rownames(ttNW_parent) = gene_anno[match(rownames(ttNW_parent), gene_anno$ensembl_gene_id), ]$hgnc_symbol
write.table(ttJAX[, 1:6], file = file.path(personal_path, paste0("/publication/", "GCM1_DE_JAX.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttMSK[, 1:6], file = file.path(personal_path, paste0("/publication/", "GCM1_DE_MSK.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttUCSF[, 1:6], file = file.path(personal_path, paste0("/publication/", "GCM1_DE_UCSF.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttNW[, 1:6], file = file.path(personal_path, paste0("/publication/", "GCM1_DE_NW.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttNW_parent[, 1:6], file = file.path(personal_path, paste0("/publication/", "GCM1_DE_NW_parent.csv", sep='')), 
            sep = "\t", quote = FALSE)
...
make_pair_plots <- function(A, B, nameA, nameB, logFC_col = 1, padj_col = 5, base_size = 12) {
...
for (ab in pairs) {
  ...
  # Save p-value plot
  ggsave(
    filename = file.path(personal_path, paste0("publication/", sprintf("GCM1_padj_%s-%s.png", nameA, nameB))),
    plot     = plots$p_padj,
    device   = ragg::agg_png,
    bg       = "transparent",
    width    = width_px,
