...
ggsave(
  filename = file.path(personal_path, paste0("/publication/", "PCPlot_selected_samples_detailed.png",sep='')),
  plot     = gPC,
  device   = ragg::agg_png,     # AGG backend, full alpha support
  bg       = "transparent",
  width    = width_px,
...
write.table(ttJAX[, 1:6], file = file.path(personal_path, paste0("/publication/", "JAX_DE_JAX.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttMSK[, 1:6], file = file.path(personal_path, paste0("/publication/", "JAX_DE_MSK.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttUCSF[, 1:6], file = file.path(personal_path, paste0("/publication/", "JAX_DE_UCSF.csv", sep='')), 
            sep = "\t", quote = FALSE)
write.table(ttNW[, 1:6], file = file.path(personal_path, paste0("/publication/", "JAX_DE_NW.csv", sep='')), 
            sep = "\t", quote = FALSE)
```
...
```{r}
ttJAX = as.data.frame(data.table::fread(file.path(personal_path, paste0("/publication/", "JAX_DE_JAX.csv", sep=''))))
rownames(ttJAX) = ttJAX$V1
ttMSK = as.data.frame(data.table::fread(file.path(personal_path, paste0("/publication/", "JAX_DE_MSK.csv", sep=''))))
rownames(ttMSK) = ttMSK$V1
ttUCSF = as.data.frame(data.table::fread(file.path(personal_path, paste0("/publication/", "JAX_DE_UCSF.csv", sep=''))))
rownames(ttUCSF) = ttUCSF$V1
ttNW = as.data.frame(data.table::fread(file.path(personal_path, paste0("/publication/", "JAX_DE_NW.csv", sep=''))))
