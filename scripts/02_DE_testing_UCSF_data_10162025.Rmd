...
names(my_colors) = c('AID_OsTIR1', 'CRISPRi_WT', 'CRISPRi_DMSO', 'CRISPRi_AAVS1', 'AID_NW', 'AID_DMSO', 'AID_WT', 'KO_JAX', 'KO_WT', 'CRISPRi_UCSF',        'KO_MSK')
```
...
shape_lookup <- ifelse(PC_df$KO_WT == "KO", 10, 23)
...
cts_dat_m = cts_dat_n[meta$Center != "MSK", ]
meta_m = meta[meta$Center != "MSK", ]
...
ggsave(
  filename = file.path(personal_path, paste0("/publication/", "UCSF_VolcanoPlot_MSK.png",sep='')),
  plot     = p4,
  device   = ragg::agg_png,     # AGG backend, full alpha support
  bg       = "transparent",
  width    = width_px,
...
write.table(ttNW[, 1:6], file = file.path(personal_path, paste0("/publication/", "UCSF_DE_NW.csv", sep='')),
            sep = "\t", quote = FALSE)
write.table(ttNW_dmso[, 1:6], file = file.path(personal_path, paste0("/publication/", "UCSF_DE_NW_DMSO.csv", sep='')),
            sep = "\t", quote = FALSE)
write.table(ttMSK[, 1:6], file = file.path(personal_path, paste0("/publication/", "UCSF_DE_MSK.csv", sep='')),
            sep = "\t", quote = FALSE)
```
