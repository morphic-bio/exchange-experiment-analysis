...
gPC = ggplot(PC_df, aes(x = PC1, y = PC2, color=new_condition, shape = KO_WT)) +
...
ggsave(
  filename = file.path(personal_path, paste0("/publication/", "NKX2-1_PCPlot_WT_samples.png",sep='')),
  plot     = gPC,
  device   = ragg::agg_png,     # AGG backend, full alpha support
  bg       = "transparent",
  width    = width_px,
...
model1 = c("NW_OSTIR_NKX2_1_Plus_IAA", "NW_OSTIR_NKX2_1_Plus_DMSO", "UCSF_CRISPRi_NKX2_1_guides_Plus_Dox", "UCSF_CRISPRi_NKX2_1_guides_No_Dox",
           "JAX_Clone_A06", "JAX_Clone_A03", "KOLF_Parent", 'MSK_Clone_K03')
...
gPC = ggplot(PC_df, aes(x = PC1, y = PC2, shape=KO_WT, color=new_condition)) +
...
ggsave(
  filename = file.path(personal_path, paste0("/publication/", "NKX2-1_PCPlot_selected_samples.png",sep='')),
  plot     = gPC,
  device   = ragg::agg_png,     # AGG backend, full alpha support
  bg       = "transparent",
  width    = width_px,
...
for (ab in pairs) {
  ...
  # Save p-value plot
  ggsave(
    filename = file.path(personal_path, paste0("publication/", sprintf("NKX2-1_padj_%s-%s.png", nameA, nameB))),
    plot     = plots$p_padj,
    device   = ragg::agg_png,
    bg       = "transparent",
    width    = width_px,
...
rownames(ttMSK)  <- map_symbols(rownames(ttMSK),  gene_anno)
rownames(ttNW_dmso)  <- map_symbols(rownames(ttNW_dmso),  gene_anno)

write.table(ttJAX[, 1:6], file = file.path(personal_path, paste0("/publication/", "NKX2-1_DE_JAX.csv", sep='')),
            sep = "\t", quote = FALSE)
write.table(ttUCSF[, 1:6], file = file.path(personal_path, paste0("/publication/", "NKX2-1_DE_UCSF.csv", sep='')),
            sep = "\t", quote = FALSE)
write.table(ttNW[, 1:6], file = file.path(personal_path, paste0("/publication/", "NKX2-1_DE_NW.csv", sep='')),
            sep = "\t", quote = FALSE)
write.table(ttNW_dmso[, 1:6], file = file.path(personal_path, paste0("/publication/", "NKX2-1_DE_NW_DMSO.csv", sep='')),
            sep = "\t", quote = FALSE)
write.table(ttMSK[, 1:6], file = file.path(personal_path, paste0("/publication/", "NKX2-1_DE_MSK.csv", sep='')),
            sep = "\t", quote = FALSE)
```
...
ttNW = as.data.frame(data.table::fread(file.path(personal_path, paste0("/publication/", "NKX2-1_DE_NW.csv", sep=''))))
